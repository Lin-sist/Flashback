...
- [ ] **T-31** 回看对话真实观感（气质是否越界、是否话痨）
- [ ] **T-32** **时间归属护栏在高频复述场景的误伤率实测** —— 本刀对 R8 的核心贡献。若误伤严重，作为**校准单独请示**，不在实现期自行调松
- [ ] **T-33** 观察模型是否在回看中尝试 tool_calls，fail-closed 是否被真实触发
- [ ] **T-34** 顺延执行 C3a 的 T-20~T-23，结论**新增条目**补记进 C3a closeout（不改写历史）
- [ ] **T-35** **R3 补齐**：微信真机手验回看浮层 + C2 遗留的工具链路端到端
- [ ] **T-36** 拦截方向若仍未复现，**诚实记为未活体验证**

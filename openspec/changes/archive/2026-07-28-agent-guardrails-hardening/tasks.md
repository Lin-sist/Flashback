...
- [x] T-43 取得用户明确 **外调授权**（闸门 3）（2026-07-28）；**已事前确认 `AI_PROVIDER` 与 model 取值后才联调**（C1 流程偏差未重演）
  - 实际用量：**4 次外调 / 预算 30**
- [ ] T-43b R6 凭证轮换（**用户执行，仍未完成**）；本轮联调只把 `AI_API_KEY` 注入进程环境变量，未打印取值
- [ ] T-44 启动本地 MySQL80 —— **未执行**：本轮探针不写库（只调 provider + 内存判定），无需数据库
- [x] T-45 真实 provider 复现 R1 场景
  - provider 可达性 PASS；**但增写行为未复现**（三次运行模型均未增写，`toolCalls=0` / 素材 coverage=1.000）
  - 结论已如实写入 design §3.3「闸门 3 真实 provider 观察结果」：**误伤方向已验证，拦截方向未活体验证**
- [x] T-46 阈值校准：4 个真实样本全部判忠实且离阈值有余量（maxRun 0 与 3，上限 12），**未触发放宽阈值的需要**；忠实度闸未被关闭
- [ ] T-47 微信小程序手验 —— **用户决定本轮不做**（优先快速完成阶段迭代）
- [ ] T-48 补齐 C2 遗留 T-40~T-42（R3）—— **随 T-47 一并延后**，仍挂在 carry-over
- [x] T-49 全部验证结果与校准结论已追加 `.ai/AGENT_LOG.md`
- [x] T-49b 新增 `C4RealProviderProbeTest`：由 `C4_REAL_PROBE=1` 环境变量门控，**默认跳过**，避免探针混进常规回归产生意外外调；作为后续阶段可复用的闸门 3 资产

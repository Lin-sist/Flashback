...
> 状态日期：2026-07-27  
> 状态：**已冻结 v1.1**（2026-07-27 用户确认冻结；修改序列须显式修订并更新 §7）  
...
- **可调整**：若联调中出现 Agent 气质越界或行为失控问题，允许将 **C4 前移**。顺序调整须更新本文 §7 修订记录。
  - **已发生（2026-07-28）**：C2 闸门 3 取得气质越界实证，C4 已前移至 C3 之前。当前执行顺序为 C1 → C2 → **C4** → C3 → C5。详见 §7。
...
| v1.1 | 2026-07-29 | C3 拆分登记（不改冻结方向） | **C3 `agent-memory-and-review` 依 §4「可选拆分退路」拆为两刀**（2026-07-29 用户批准）：① `agent-memory-retrieval`——Memory 检索 + 写作引导注入 + `MemoryPort` + 跨记录关联能力 + 分层来源 + 时间归属护栏；② `agent-review-chat`——友人回看会话（复用 `agent_session` + `purpose`）+ `ReviewChatSheet` 浮层，消费同一 `MemoryPort`。拆分理由：两个子能力改动面几乎不重叠（前者集中在上下文组装与 `guardrail` 包，后者集中在会话模型、状态机、已接受 spec 条款与新前端浮层），合并会产出过长且互相阻塞的 tasks；先做 Memory 是因为回看对话的价值全在「Agent 记得你」，它必须消费 Memory 才成立。执行顺序变为 C1 → C2 → C4 → **C3a → C3b** → C5，一次仍只 ACTIVE 一个。方向、依赖规则与气质约束均未修改。**同时登记一项规划期核实的事实修正**：§4 C3「现状事实」曾称「现有 `summarizeRecord` 已对记录生成结构化摘要，可作为 Memory 检索索引」——实际 `record.ai_summary` 由前端在创建/更新时回传写入，后端**不会**自动为每条记录生成，故该字段的实际覆盖率为 `unknown`，检索设计已改为不依赖单一字段 |
| v1.1 | 2026-07-28 | 顺序调整（不改冻结方向） | **C4 前移至 C3 之前**，执行顺序变为 C1 → C2 → **C4** → C3 → C5。依据 §3.2「若出现 Agent 气质越界或行为失控则允许将 C4 前移」＋ 2026-07-28 用户批准。触发实证：C2 闸门 3 真实联调中，工具参数 `text` 增写了用户从未说过的整句、`askText` 自称「我帮你整理了一下」，触碰「不改写用户原文」硬约束；C2 的白名单与二段式确认只校验「能否执行」，不校验「参数内容是否忠实」（C2 design 决策 7 划定的边界）。C2/C3/C4/C5 彼此无硬依赖、均只硬依赖 C1，故依赖链未被破坏。方向、依赖规则与气质约束均未修改，仅调整执行次序 |

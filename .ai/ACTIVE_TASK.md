...
- **[已修复] 对话气泡显示 JSON 原文**：回复呈现为 `{"reply":"..."}`。根因＝C2 把解析路径从 `extractText(raw,"reply")` 改为直接读 `message.content`，但 `buildSystemPrompt` 仍要求模型「只输出 JSON」，模型照做而后端不再剥壳。修复＝改写输出要求为直出自然语句 + 新增 `normalizeReplyShape` 形状兜底 + 回归守门测试。**属 C2 引入的缺陷，非模型问题**
...
- **[待优化，不在 C2 范围] 素材过于生硬**：`buildMaterialMessages` 现要求「只使用用户说过的内容 / 尽量保留用户自己的措辞」，模型将其执行为**逐句拼接原话**。用户口语是断断续续的，直接拼接会导致句子逻辑不连贯。
